...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async initData(){
        if (this.userInfo) {
          this.hongbaoList = await
            getHongbaoNum(this.userInfo.user_id);
...
<style lang="scss" scoped>
  ...
  span:nth-of-type(1) {
  @include sc(.75 rem, #ff5340);
    font-weight: bold;
  }

  span:nth-of-type(2) {
  @include sc(1.5 rem, #ff5340);
  }

  span:nth-of-type(3), span:nth-of-type(4) {
  @include sc(0.8 rem, #ff5340);
    font-weight: bold;
  }

  p {
  @include sc(0.4 rem, #999);
  }
