...
let Axios_Config = {
  ...
  data: {
    ...
    gzip: 0,
    client: 4,
    'client_type': config.client[channel]
  },
...
function commonResponseHandler(resp) {
  switch (resp.code) {
    case 0:
      return true;
      break;
...
let request = (data = {}, url, method = 'POST') => {
  ...
  if (method === 'POST') {
    return Axios.post(
      url, data, Axios_Config
    ).then(function (response) {
      let resp = response.data;
      if (commonResponseHandler(resp)) {
        return resp;
      }
    }).catch(function (err) {
      console.log(err);
...
/*获取短信验证码*/
export const getMobileCode = data => request(data, config.url.api, "POST");

/*登录*/
export const sendLogin = data => request(data, config.url.api, "POST");
